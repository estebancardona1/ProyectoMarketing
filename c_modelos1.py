...
datos['title'] = datos['title'].apply(fix_titles)

display(datos)

# Guardar el dataframe final
joblib.dump(datos,"salidas\\datos_movies.joblib")
...
top_a = pd.read_sql("""SELECT title, year, 
...
top_b = pd.read_sql("""SELECT title, year, 
                ROUND(AVG(rating), 2) AS score,
                COUNT(*) as views
                FROM full_ratings
                GROUP by title
...
top_c = pd.read_sql('''  
            SELECT title, year,
            COUNT(fr.movieId) AS views,
            (SELECT ROUND(AVG(rating), 2) 
             FROM full_ratings 
             WHERE movieId = fr.movieId) AS score
            FROM full_ratings fr
...
option_dropdown = widgets.Dropdown(
    options=options,
    description='Elige:',
    value='Top 10 Más Vistas Último Año'  # Valor por defecto
)
...
def show_top(selected_option):
    ...
    if selected_option == 'Top 10 Mejores Calificadas Global':
        top_a['title'] = top_a['title'].apply(fix_titles)
        ...
        top_b['title'] = top_b['title'].apply(fix_titles)
        ...
        top_c['title'] = top_c['title'].apply(fix_titles)
        ...
        top_d['title'] = top_d['title'].apply(fix_titles)
...
def show_top_movies(selected_genre):
    ...
    genre_movies = datos[datos[selected_genre] == 1]
    ...
    top_movies = genre_movies.groupby(['title', 'year']).agg(
        score=('rating', 'mean'),  # Promedio de ratings
        views=('rating', 'count')  # Contar el número de calificaciones
    ).reset_index()
    ...
    top_movies = top_movies[top_movies['views'] >= 20]
...
def recomendacion(movie=sorted(list(movies_sep['title']))):
    ...
    top_similar_movies = similar_movies.to_frame(name="corr").round(3)
    ...
    return top_similar_movies[['title', 'year', 'corr']]
...
def MovieRecommender(movie=sorted(list(movies_sep['title'].value_counts().index))):
    ...
    movie_recommendations_df = pd.DataFrame(movie_list, columns=['title', 'year', 'dist'])
    
    # Ordenar las recomendaciones por las distancias más cercanas
    movie_recommendations_df = movie_recommendations_df.sort_values(by='dist', ascending=True).reset_index(drop=True)
    
    # Obtener la primera recomendación
    top_movie = movie_recommendations_df.iloc[0]['title']
